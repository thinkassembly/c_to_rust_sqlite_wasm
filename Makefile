...
OUTFILES_BROWSER := c_to_rust_sqlite_wasm_bg.wasm c_to_rust_sqlite_wasm_bg.js c_to_rust_sqlite_wasm_bg.d.ts
OUTPATHS_BROWSER := $(foreach file,$(OUTFILES_BROWSER),$(DIST_BROWSER)/$(file))
DIST_NODEJS := dist-nodejs
OUTFILES_NODEJS := $(OUTFILES_BROWSER) c_to_rust_sqlite_wasm_bg.js
...
node-debug: src/* libs/libc-sys
	...
	$(DOCKER_RUN)  /bin/bash -c "wasm-opt /c/dist-nodejs/c_to_rust_sqlite_wasm_bg.wasm -o /c/dist-nodejs/c_to_rust_sqlite_wasm_bg.wasm"
...
browser-debug: src/* libs/libc-sys
	...
	$(DOCKER_RUN)  /bin/bash -c "wasm-opt /c/dist-browser/c_to_rust_sqlite_wasm_bg.wasm -o /c/dist-browser/c_to_rust_sqlite_wasm_bg.wasm"
...
node-test: src/* libs/libc-sys
	mkdir -p target
	$(DOCKER_RUN)  /bin/bash -c "/clang/bin/llvm-ar d /musl-sysroot/lib/libc.a memcpy.o memmove.o memset.o expf.o memcmp.o fmax.o fmin.o  && /.cargo/bin/wasm-pack test  --node -- --lib --color always"
